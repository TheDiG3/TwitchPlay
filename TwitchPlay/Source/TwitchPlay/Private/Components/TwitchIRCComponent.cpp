...
{
	FResolveInfo* resolve_info = ISocketSubsystem::Get()->GetHostByName( "irc.twitch.tv" ); // Name resolution for Twitch IRC server
	while ( !resolve_info->IsComplete() ); // Wait for resolution completion

	if ( resolve_info->GetErrorCode() != 0 )
	{
		_out_error = "Could not resolve hostname!";
		return false; // if the host could not be resolved return false
	}

	// Now we need to get the resolved IP in order to use it for the actual connection
	const FInternetAddr* addr = &resolve_info->GetResolvedAddress();
	uint32 out_IP;
	addr->GetIp( out_IP );
	int32 port = 6667; // Standard IRC port

	// Creates the connection info for the socket based on the received resolve info
	TSharedRef<FInternetAddr> connection_addr = ISocketSubsystem::Get( PLATFORM_SOCKETSUBSYSTEM )->CreateInternetAddr();
	connection_addr->SetIp( out_IP );
	connection_addr->SetPort( port );

	// Creates a new socket connection. Deletion of said socket is assigned to external users
	FSocket* ret_socket = ISocketSubsystem::Get( PLATFORM_SOCKETSUBSYSTEM )->CreateSocket( NAME_Stream , TEXT( "TwitchPlay Socket" ) , false );
	...
	{
		ret_socket->Close();
		ISocketSubsystem::Get( PLATFORM_SOCKETSUBSYSTEM )->DestroySocket( ret_socket );
